...
// Options contains the options for deploying a project.
type Options struct {
	ProjectPath string
	Register    bool
	DryRun      bool
...
// Run executes the deploy process: build image and register with orchestrator.
func Run(opts Options) error {
	...
	if opts.DryRun {
		...
		if opts.Register {
			fmt.Println("Would register deployment:", cozyConfig.DeploymentID)
		}
		return nil
	}
	...
	// Register deployment if requested
	if opts.Register {
		fmt.Println("\nRegistering deployment with orchestrator...")
